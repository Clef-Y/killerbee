...
/* Sub-1GHz (915 MHz Wi-SUN, NOT O-QPSK - see kb_cc1352p7.syscfg's
 * RF_Settings_SUBG_OQPSK comment for why this chip uses a different
 * sub-1GHz PHY than the CC1354P10's) globals. Note RF_cmdPropTx here is
 * actually a CMD_PROP_TX_ADV (rfc_CMD_PROP_TX_ADV_t), not the plain
 * CMD_PROP_TX the CC1354P10 generates - this Wi-SUN preset only exposes
 * the ADV TX command. Confirmed field-compatible with this file's use
 * (only .pktLen/.pPkt are touched, both present and identically named on
 * CMD_PROP_TX_ADV - see rf_prop_cmd.h), so no call-site changes needed
 * beyond this alias. */
#define RF_propSubg           RF_prop_2gfsk50kbps25dev915wsun1b_1
#define RF_cmdPropRadioSetup  RF_cmdPropRadioDivSetup_2gfsk50kbps25dev915wsun1b_1
#define RF_cmdFsSubg          RF_cmdFs_2gfsk50kbps25dev915wsun1b_1
#define RF_cmdPropTx          RF_cmdPropTxAdv_2gfsk50kbps25dev915wsun1b_1
#define RF_cmdPropRx          RF_cmdPropRx_2gfsk50kbps25dev915wsun1b_1
...
static const char FW_ID[] = "KB-CC1352P7 v1.0";
 ...
 * documented in the README. Symbol addresses: `tiarmnm kb_cc1352p7.out
 ...
 * fixed by reducing sub-1GHz TX power to 0 dBm in kb_cc1352p7.syscfg.
 ...
 * by reducing sub-1GHz TX power to 0 dBm in kb_cc1352p7.syscfg. Retested
...
void *mainThread(void *arg0)
{
     ...
     * main_tirtos.c - checked against the sibling CC1354P10 target this
     * fix was originally found on, not re-verified against an equivalent
     * CC1352P7 example, but the same CC13x2/CC13x4 prop-RF core
     * constraints apply to both) sets exactly these two constraints -
     * gated behind a different board's config flag
     * (CONFIG_LP_CC2674R10_FPGA) rather than applied generally -
     * confirming this is a real, TI-acknowledged necessity for continuous
     * prop-RF activity on this chip family, not a guess. The default idle
     * policy (PowerCC26XX_standbyPolicy, see
