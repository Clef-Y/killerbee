 ...
 *
 * Ported from ../kb-cc1354p10/main.c, which is the project's primary,
 * hardware-validated target (see that file's header for its stage-by-stage
 * validation history). This file is source-identical to it except for this
 * header, FW_ID below, and DeviceFamily_constructPath()/ti_radio_config.h
 * resolving to the CC1352P7's own headers at build time - it only touches
 * rfc_CMD_IEEE_RX_t/CMD_IEEE_TX_t fields common to both chip families (the
 * CC1354P10's cc13x4_cc26x4 headers add dual-PAN fields the CC1352P7's
 * cc13x2x7_cc26x2x7 headers don't have; this firmware uses neither), so no
 * other source changes were needed for it to build. It has NOT yet been
 * independently hardware-validated on a CC1352P7 board - treat every stage
 * as unverified here until it has been.
 *
 * Hardware: this targets the CC1352P7 LaunchPad's own ONBOARD XDS110 debug
 * probe (2-pin cJTAG, not the standalone LP-XDS110 + standard-JTAG pairing
 * the CC1354P10 firmware uses) - see README.md.
 *
 ...
 * RF_pendCmd(..., RF_EventLastCmdDone): on the CC1354P10 that blocking wait
 * was observed to hang indefinitely even after the command had already
 * completed successfully, wedging the whole UART command loop. Carried over
 * here defensively even though it hasn't been specifically confirmed to
 * reproduce on this chip.
 */
