...
**Minimum reliable `--dwell` (measured):** pushed down from the 20ms
example above to find the actual floor, same independent-RSSI-monitor
method (CC1354P10 running `rssi_scan.py`, channels 11/15/20/25). 3ms
dwell is clean and strong on every channel across repeated trials (~-57
to -68 dBm, 32-43 dB above baseline); 2ms still works but with less
margin (~-74 to -82 dBm, 22-30 dB above baseline). 1ms - the wire
protocol's own floor (`dwell_ms` is a `uint16` and `0` is rejected) - is
marginal: at least one of the four channels came back measurably weaker
or fully indistinguishable from noise in every 1ms trial run, consistent
with `rfTuneToChannel()`'s own ~1ms-worst-case retune cost being the same
order of magnitude as the requested dwell at that point, so actual
per-channel on-air time becomes inconsistent hop to hop rather than a
clean floor/ceiling cutoff. No hang/wedge from the hop itself at any of
these dwells - `PING`/`GET_CHANNEL` stayed responsive throughout - though
heavy back-to-back low-dwell runs did eventually trigger the documented
"RF core can get stuck after heavy use" state once, recovered cleanly via
`tools/kb_jtag_recover.sh`. **Treat 3ms as the shortest `--dwell` to rely
on; 2ms works with reduced margin; below that is not recommended.**
