...
def iscc1352p7(serialdev: str) -> bool:
    '''
    Determine if a given serial device is a TI CC1352P7 running the
    KillerBee firmware in firmware/src/kb-cc1352p7 (reached over the
    LaunchPad's onboard XDS110 debug probe's auxiliary backchannel UART).
    @type serialdev: String
    @param serialdev: Path to a serial device, ex /dev/ttyACM0.
    @rtype: Boolean
    '''
    ...
    try:
        ...
        if len(hdr) != 2 or hdr[0] != 0x81:  # CMD_PING | CMD_REPLY_BIT
            return False
        payload = s.read(hdr[1])
        return payload.startswith(b"KB-CC1352P7")
    except serial.serialutil.SerialException:
        return False
    finally:
        s.close()
